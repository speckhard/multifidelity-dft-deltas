...
flags.DEFINE_string(
    ...
    'CSV file path from which to add edit columns from the df')
flags.DEFINE_string(
    'dft_code_name',
    'fhi_aims',
    'DFT-code-name')
...
class FeatureEng():
        ...
        self.df['a_basis_functions'] = [
            basis_dict[get_aims_basis_set_size(y)[0], x,
                get_aims_basis_set_size(y)[1]] for x, y in zip(
                    self.df['a_atom_num'], self.df['binary_precision'])]

        self.df['b_basis_functions'] = [
            basis_dict[
                get_aims_basis_set_size(y)[0], x,
                get_aims_basis_set_size(y)[1]] for x, y in zip(
                    self.df['b_atom_num'], self.df['binary_precision'])]
    ...
    def basis_fxn_per_val(self):
        ...
        self.df['numerical_setting_int'] = [
            convert_num_setting_2_int(x) for x in self.df[
                    'num_setting']]

        self.df['basis_set_size_int'] = [
            convert_basis_set_2_int(x) for x in self.df[
                'basis_size']]
...
def convert_basis_set_2_int(basis_set):
    if basis_set == 'minimal':
        num_int = 0
    elif basis_set == 'tier1':
        num_int = 1
    elif basis_set == 'standard':
        num_int = 2
    elif basis_set == 'tier2':
        num_int = 3
    else:
        print('Wrong basis_set setting % s' % basis_set)
        print(f'basis_size type {type(basis_set)}')
        print(basis_set == 'minimal')
        sys.exit(0)
    return num_int
...
def main(argv):
    ...
    valence_csv_filename = csv_filename
    feat_eng_csv_filename = (
        BASE_FOLDER +
        '/parsing/data/feat_eng/' + dft_code_name +
        '_geo_opt_prepped_w_valence_w_feat_eng' + csv_suffix + '.csv')
    # Create object
