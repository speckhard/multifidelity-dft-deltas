...
flags.DEFINE_string('root_dir', '/u/dansp/egnn/settings_only_sweep', 'Root directory.')
...
flags.DEFINE_string('wandb_project', 'egnn-settings-only', 'WandB Project.')
flags.DEFINE_string('wandb_group', 'settings_only_sweep_v1', 'WandB Group.')
...
# Settings-only model: use settings_only_config and set model_type
srun python3.12 /u/dansp/egnn/errorbar_modelling/modelling/gnn/train_pipeline.py \\
    --config-name=settings_only_config \\
...
def main(argv):
    ...
    print(f"Model Type: settings_only")
    ...
    for _, params in enumerate(combinations):
        ...
        job_name = f"sonly_{FLAGS.loss_function}_" + "_".join(parts)
